    ...
    function chart() {
        ...
        function faceOffCircle(x, y, group) {
            ...
            var outsideLineData = [
                ...
                {
                    x: 0,
                    y: xStartOutsideLine + RINK_CONFIG.FACEOFF_OUT_MARK_LENGTH,
                },
            ];
            faceOff
                .append("path")
            ...
            var trapezoidFunction = d3
                ...
                .y(function (d) {
                    return (
                        0.5 *
                            (RINK_CONFIG.RINK_WIDTH -
                                RINK_CONFIG.CENTER_TO_NEUTRAL_DOT) +
                        d.y
                    );
                })
                .curve(d3.curveLinear);
            ...
            var trapezoidData = [
                {
                    ...
                    y:
                        -0.5 *
                        (RINK_CONFIG.TRAPEZOID_BOTTOM -
                            RINK_CONFIG.TRAPEZOID_TOP),
                },
                { x: 0, y: 0 },
            ];
        ...
        var dStringCreator = function (input) {
            ...
            for (var i = 0; i < input.length; i++) {
                if (input[i]["type"] === "M" || input[i]["type"] === "L") {
                    dStr +=
                        input[i]["type"] + input[i]["x"] + " " + input[i]["y"];
                } else if (input[i]["type"] === "A") {
                    dStr +=
                        input[i]["type"] +
                        input[i]["radius"] +
        ...
        function dangerZones(zoneCoords, group) {
            ...
            dangerData[0] = {
                ...
                x:
                    ...
                    zoneCoords[0]["y1"] * rinkScale,
                y:
                    0.5 * RINK_CONFIG.RINK_WIDTH +
                    zoneCoords[0]["x1"] * rinkScale,
            };
            var coord;
            var i = 1;
            for (coord in zoneCoords) {
                ...
                dangerData[i]["x"] =
                    0.5 * RINK_CONFIG.RINK_LENGTH -
                    zoneCoords[coord]["y1"] * rinkScale;
                dangerData[i]["y"] =
                    0.5 * RINK_CONFIG.RINK_WIDTH +
                    zoneCoords[coord]["x1"] * rinkScale;
                dangerData[i]["type"] = "L";
                i++;
            }
